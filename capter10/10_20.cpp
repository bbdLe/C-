...
void biggies(vector<string> &vec, size_t sz)
{
	...
	auto endIter = stable_partition(vec.begin(), vec.end(), [sz](const string &str) {return str.size() >= sz;});
	auto wc = count_if(vec.begin(), vec.end(), [sz](const string &str) {return str.size() >= sz;});
