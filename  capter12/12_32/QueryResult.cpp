...
QueryResult::QueryResult(string w, shared_ptr<StrBlob> fptr, shared_ptr<set<size_type>> lptr) : word(w), file(fptr), lineNo(lptr) {}
...
ostream &print(ostream &os, const QueryResult &result)
{
	...
	for(auto bg = result.begin(), eof = result.end(); bg != eof; ++bg)
		cout << *bg + 1 << " : " << result.getFile()->get(*bg) << endl;
